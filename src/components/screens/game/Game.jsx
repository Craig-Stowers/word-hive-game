...
const Game = forwardRef(({ screen, dataIndex = 3, size }, ref) => {
    ...
    useEffect(() => {
        ...
        if (screen.globalData.localData.success[days]) {
            if (screen.globalData.localData.incomplete[days]) {
                const allIncomplete = {
                    ...screen.globalData.localData.incomplete,
                };
    ...
    useEffect(() => {
        ...
        const shuffled =
            ...
            Array.isArray(storedData.shuffledLetters) &&
    ...
    useOnKeyPress(
        handleAllowedLetter,
        [],
        [...allLettersArr, ...allLettersArr.join("").toLowerCase().split("")]
    );
    ...
    const handleEnter = () => {
        ...
        if (!trimAnswer.toLowerCase().includes(centerLetter.toLowerCase())) {
            overlayRef.current.generateAlert({
                type: "tip",
                text: "Must include centre letter",
            });
            return;
        }

        const lowerCaseCorrectWords = correctWords.map((word) =>
            word.word.toLowerCase()
        );
        if (lowerCaseCorrectWords.includes(trimAnswer.toLowerCase())) {
            overlayRef.current.generateAlert({
                type: "tip",
                text: "Word used already",
            });
            return;
        }

        if (
            availableAnswers.includes(trimAnswer) &&
            !correctWords.includes(trimAnswer)
        ) {
            ...
            const bonusCharacterCount = countOccurrences(
                trimAnswer,
                bonusLetter
            );

            newPoints += bonusCharacterCount * 5;
            const isPangram = containsAllLetters(
                trimAnswer,
                (centerLetter + letters).toLowerCase()
            );
    ...
    return (
        <div className={classes.root}>
            ...
            <OverlayPortal
                mirrorElementId={"bonus-letter"}
                deps={[shuffledLetters]}
            />
