    ...
    const revealClasses = useMemo(() => {
        ...
        for (let i = 0; i < letterArr.length; i++) {
            ...
            if (wordLetters.includes(letterArr[i].toLowerCase())) {
                const isBonus =
                    letterArr[i].toLowerCase() ===
                    answer.bonusLetter.toLowerCase();
                revealClasses.push(
                    classes[`${isBonus ? "bonus" : "show"}${i + 1}`]
                );
            }
        }
    ...
    return (
        <div className={classes.root}>
            <div
                ...
                style={{ opacity: answer ? 1 : 1 }}
            >
                <img src={answer ? filledJar : emptyJar} />
                {/* <Honey /> */}
                <div
                    className={`${classes.gridWrap} ${revealClasses.join(" ")}`}
                >
                    <Grid />
                </div>
            </div>
