...
const OverlayAlerts = forwardRef((props, ref) => {
    ...
    const tips = alertObjects.filter(
        (alert) => alert.type === "tip" || alert.type === "points"
    );
    const bonus = alertObjects.filter((alert) => alert.type === "bonus");
    const middleBonus = alertObjects.filter(
        (alert) => alert.type === "mid-bonus"
    );
        ...
        <>
            ...
            <Portal id={"answer-box-portal"}>
                {tips.map((alert, index) => {
                    const config =
                        alert.type === "points" ? pointsConfig : tipConfig;
                    return (
                        <div
                            style={{
                                fontFamily: "'Roboto Condensed', serif",
